...
| 5 自動テスト | **実装完了（2026-09-22。Windows 実機は未確認）** | `packages/test` Vitest 45 件（sqlite の DB ヘルパ、ヘッドレス Chromium を代役にした CDP 起動 / 契約呼び出し）、`apps/web` の screen 6 件が Linux（Claude Code on the web）で通過。雛形は tgz から scaffold → install → gen:check → web typecheck → screen 4 件通過。api / host は会社 PC で `pnpm --filter web test:doctor` → `test:e2e` |
...
7. 自動テスト（2026-09-22、§10 参照）: `packages/test` と雛形の `e2e/` を実装。**次は会社 PC で** `git pull` → `pnpm install` → `dotnet build dotnet/WebView2Bridge.sln`（Debug）→ `pnpm --filter web test:doctor` → `pnpm --filter web test:e2e`（api 4 件 + host 2 件）。通ったら 0.5.0 として npm 4 つを公開し、雛形から作った新規アプリで `pnpm test:all` まで確認する。DB ヘルパの SQL Server / Oracle 実機確認はテスト DB のある業務アプリで行う（`.env.e2e.local` と `db.allowedDatabases`）
