...
class VbContext {
  ...
  renderDispatcher(): string {
    ...
    out.push(`Namespace Global.${this.options.namespace}`);
    out.push("");
    out.push(`    Partial Public Class ${this.dispatcherClass}`);
    out.push("");
    out.push(`        ''' <summary>契約に含まれる全メソッド名</summary>`);
    out.push(
      `        Public Shared ReadOnly MethodNames As String() = {${this.methods.map((m) => vbString(m.rpcName)).join(", ")}}`,
    );
    for (const ns of this.namespaces()) {
      out.push("");
      out.push(`        ''' <summary>"${ns}.*" の実装を登録する</summary>`);
      out.push(`        Public Sub Register(api As ${interfaceName(ns)})`);
      out.push(`            If api Is Nothing Then Throw New ArgumentNullException(NameOf(api))`);
      for (const m of this.methods.filter((x) => x.ns === ns)) {
        out.push(
          `            RegisterHandler(Of ${m.request}, ${m.response})(${vbString(m.rpcName)}, AddressOf api.${vbEscape(m.vbName)})`,
        );
      }
      out.push("        End Sub");
    }
